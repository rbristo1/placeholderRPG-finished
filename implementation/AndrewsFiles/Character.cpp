...
Character::Character(int id, string characterName, const int* statArray){// Custom constructor (used in player and enemy constructors)
    srand(time(NULL));
...
void Character::heal(int amount) {
    hp += amount;
    if (hp > stats[0]) { // Make sure player can't go over MAX_HP
        hp = stats[0];
    }
}
...
void Character::changeStats(int stat, int amount, bool permenant) {
    if (permenant) { // If permanent, then add it to the normal stats, as temp stats get reset out of combat, and are used for stat boosting moves and potions
        stats[stat] += amount;
    } else {
...
string Character::applyStatus(int id, float modifier, bool caster) { // Either does the effects of a one time status, or sets status of character to index other than -1
    ...
    // Apply Paralasis
    if (id == 10 && status == -1 && !caster) { // modifier is the percent chance that paralysis is applied
    ...
    // Apply Bleed
    if (id == 11 && status == -1 && !caster) { // modifier is the percent chance that bleed is applied
        int chance = rand() % 100;
        if (chance < modifier) {
            status = 1;
    ...
    // Apply Burn
    if (id == 12 && status == -1 && !caster) { // modifier is the percent chance that burn is applied
        int chance = rand() % 100;
        if (chance < modifier) {
            status = 2;
    ...
    // Slow
    if (id == 15 && status == -1 && !caster) { // modifier is the percent chance that slow is applied
...
string Character::activateStatus(bool &effect) { // Actually activates the effects of the long-term statuses
    // Paralysis
    if (status == 0) {
        int chance = rand() % 4; // Only full para 25% of the time
        if (chance == 1) {
            effect = true; // Used in battle to prevent character from moving
    ...
    // Bleeding
    if (status == 1) {
        takeDamage((int) stats[0] * 0.125); // Take damage based on health
    ...
    // Burn
    if (status == 2) {
        takeDamage((int) stats[0] * 0.125); // Take damage based on health; identical to bleed, just named something else
        string amount = to_string((int) (stats[0] * 0.125));
        if (amount == "0") {
            amount = "1";
        }
    ...
    // Slow
    if (status == 3) { // Switches between status 3 and 4 every turn, player loses turn on status 3
        status = 4;
        effect = true;
