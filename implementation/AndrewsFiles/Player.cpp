...
void Player::resetBasicAttack(){ // Used after gain a weapon, change damage of base attack to match weapon
    actions[0].setPower(weapon.getDamage());
    for (int i = 1; i < 4; i++) {
        if (actions[i].getID() == 2) { // Change damage of reckless attack to match power if known
            actions[i].setPower(weapon.getDamage() + 3);
        }
    }
}

string Player::learnNewMove(int toLearn){ // Add a move to the list of actions the player can use
    int index = -1;

    for (size_t i = 0; i < unlearnedActions.size(); i++) { // Check if the player can learn the move based on the class
    ...
    for (int i = 0; i < 4; i++) { // Find an empty action slot
        if (actions[i].getID() == -1) {
            actions[i].reconstruct(toLearn); // Set the action to the correct one
            if (toLearn == 2) {
                actions[i].setPower(weapon.getDamage() + 3); // Set up damage of reckless attack if that is what was learned
            }
            numLearned++;
...
void Player::print() { // Used for testing only
...
string Player::gainItem(int ID) {
    int index = -1;
    for (int i = 0; i < 4; i++) { // Find empty inventory slot and put item in it
...
string Player::useItem(int index, int battle) {
    ...
    if (inventory[index].getID() != -1) { // Only do something if 
        ...
        if (effect == 1 && battle == 1) { // Throw away item if not in battle
        ...
        if (effect == 2 && battle == 1) { // Throw away item if not in battle
            returnText = name + " trashed " + inventory[index].getName() + "! ";
        }
        ...
        if (effect == 3 && battle == 1) { // Throw away item if not in battle
            returnText = name + " trashed " + inventory[index].getName() + "! ";
        }
        ...
        if (effect == 4 && battle == 1) { // Throw away item if not in battle
            returnText = name + " trashed " + inventory[index].getName() + "! ";
        }
        ...
        inventory[index].reconstruct(-1); // Empty inventory slot
...
string Player::getNextWeapon() { // Reconstruct weapon to next ID
...
string Player::gainRandomPotion() { // Choose a random potion, with a 50% chance of it being a health potion, as it is the most important one
    int temp = rand() % 2;
    if (temp == 0) {
        return gainItem(0);
    } else {
